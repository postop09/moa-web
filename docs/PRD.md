...
| 주말·공휴일 표시 | 토·일 요일 헤더와 그리드 날짜 숫자를 빨간색으로 강조, 셀에 옅은 붉은 배경 틴트, 일 상세 패널 제목에 공휴일명 표시 |
| 날짜 그리드 격자 | 날짜 셀을 간격 없이 배치하고 옅은 회색 격자선으로 구분 (일정 바 영역은 가로선만)                                  |
...
- **FR-CALENDAR-3** 주말·공휴일 배경 틴트와 날짜 숫자 색상은 이번 달 날짜(`inMonth`)에만 적용한다 — 전/다음 달 패딩 날짜는 대비 저하를 피하기 위해 제외한다. 셀 배경은 항상 `--color-surface` 기준으로 합성해 격자선 색과 섞이지 않도록 한다(재사용 시 배경이 달라지면 대비 재검증 필요).
