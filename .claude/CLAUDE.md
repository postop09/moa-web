...
> 전체 철학과 배경은 공식 문서([feature-sliced.design](https://feature-sliced.design/), v2.1 기준)를 참고할 수 있다.
> 레이어 구성·세그먼트 이름·Public API 규칙은 공식 스펙과 동일하다. 다만 이 저장소는
> `eslint-plugin-boundaries`를 설치하지 않아 아래 규칙은 린트가 아닌 컨벤션으로만 지켜지고,
> 슬라이스 폴더명은 공식 예시(kebab-case)와 달리 의도적으로 camelCase를 쓰며, `entities`/`features`/
> `widgets`의 세부 기준 일부는 이 저장소만의 추가 규칙이다. 아래 내용과 공식 문서가 어긋나 보이면
> 이 저장소에서는 항상 아래 내용(및 `.claude/rules/*.md`)을 우선한다.
...
새 기능·변경 요구가 들어오면 `feature-workflow` skill의 절차를 따른다.
